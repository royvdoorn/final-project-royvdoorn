...
class SegNet(nn.Module):
    def __init__(self):
        ...
        # Encode stage 6
        self.enc_6a = nn.Conv2d(512, 1024, kernel_size=3, padding=1)
        self.enc_6b = nn.Conv2d(1024, 1024, kernel_size=3, padding=1)
        self.enc_6c = nn.Conv2d(1024, 1024, kernel_size=3, padding=1)
        self.norm_enc_6a = nn.BatchNorm2d(1024, momentum=0.5)
        self.norm_enc_6b = nn.BatchNorm2d(1024, momentum=0.5)
        self.norm_enc_6c = nn.BatchNorm2d(1024, momentum=0.5) 

        # Decode stage 6
        #self.up_6 = nn.ConvTranspose2d(512, 512, kernel_size=2, stride=2, padding=0)    
        self.dec_6a = nn.Conv2d(2*1024, 1024, kernel_size=3, padding=1)
        self.dec_6b = nn.Conv2d(1024, 1024, kernel_size=3, padding=1)
        self.dec_6c = nn.Conv2d(1024, 512, kernel_size=3, padding=1)
        self.norm_dec_6a = nn.BatchNorm2d(1024, momentum=0.5)
        self.norm_dec_6b = nn.BatchNorm2d(1024, momentum=0.5)
    ...
    def forward(self, x):
        ...
        # Decode Stage 6
        x = self.unpool(x, ind6, output_size=size5)
        #x = self.up_6(x)
